...
describe Activation::ActivationDomain do
    ...
    context '正常系' do
      context '有効なトークンを受け取った場合' do
        ...
        it 'usersのactivatedがtrueになること' do
          service = Activation::ActivationDomain.new(token)
          service.activate
    ...
    context '異常系' do
      context '不正なトークンを受け取った場合' do
        ...
        it 'Unauthorizedがスローされること' do
          service = Activation::ActivationDomain.new(token)
          expect { service.activate }.to raise_error(Activation::ActivationError::Unauthorized)
        end
      end
    end
  end
