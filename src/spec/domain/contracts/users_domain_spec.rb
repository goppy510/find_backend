  ...
  describe '#self.create' do
    context '正常系' do
      context '正しいuser_idとtarget_user_idを受け取った場合' do
        ...
        it 'ContractMembershipRepository.createが呼ばれること' do
          Contracts::UsersDomain.create(user.id, target_user.id)
          expect(ContractMembershipRepository).to have_received(:create).with(target_user.id, contract.id)
        end
      end

      context 'max_user_countに達していない場合' do
        let!(:user) { create(:user, activated: true) }
        let!(:target_user) { create(:user, activated: true) }
        let!(:contract) { create(:contract, user_id: user.id) }
    ...
    context '異常系' do
      context 'user_idが引数にない場合' do
        ...
        it 'ArgumentErrorがスローされること' do
          expect { Contracts::UsersDomain.create(nil, target_user.id) }.to raise_error(ArgumentError, 'user_idがありません')
        end
      end
      ...
      context 'target_user_idが引数にない場合' do
        ...
        it 'ArgumentErrorがスローされること' do
          expect { Contracts::UsersDomain.create(user.id, nil) }.to raise_error(ArgumentError, 'target_user_idがありません')
        end
      end
      ...
      context 'contract_idが存在しない場合' do
        ...
        it 'Forbbidenがスローされること' do
          expect { Contracts::UsersDomain.create(user.id, target_user.id) }.to raise_error(Contracts::ContractsError::Forbbiden)
        end
      end

      context 'max_user_countに達している場合' do
        let!(:user) { create(:user, activated: true) }
        let!(:target_user) { create(:user, activated: true) }
        let!(:contract) { create(:contract, user_id: user.id) }
        ...
        it 'RecordLimitExceededがスローされること' do
          expect { Contracts::UsersDomain.create(user.id, target_user.id) }.to raise_error(Contracts::ContractsError::RecordLimitExceeded)
        end
      end
    end
  end
