  ...
  describe '#find_available_user' do
    context '正常系' do
      ...
      context 'usersにレコードがあり、かつ、tokenが無効な場合' do
        let!(:token) do
          Auth::AuthTokenService.new(payload: { sub: user1.id, exp: Time.zone.local(2023, 4, 10, 3, 0, 0).to_i }).token
        end

        it 'JWT::ExpiredSignatureが発生すること' do
          expect { Auth::AuthTokenService.new(token:) }.to raise_error(JWT::ExpiredSignature)
        end
      end
    end
  end
